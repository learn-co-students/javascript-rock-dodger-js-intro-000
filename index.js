...
function endGame() {
  ...
  ROCKS.forEach(rock => {
    rock.remove(); // this removes the each rock still within the ROCKS array
  });
...
function moveDodger(e) {
  ...
  if (LEFT_ARROW === e.which) { // this conditional checks for a keydown on the LEFT_ARROW
   ...
   if (RIGHT_ARROW === e.which) { // this conditional checks for a keydown on the RIGHT_ARROW
...
function moveDodgerLeft() {
  ...
  function left() {
    var leftNumbers = DODGER.style.left.replace('px', ''); // this is the number corresponding to the coordinates of the leftmost edge of the DODGER (in pixels)
    var leftDodgerEdge = parseInt(leftNumbers, 10); // this gives the leftmost edge of the DODGER

    if (leftDodgerEdge > 0) { // this is checking to see if the leftmost edge of the DODGER has reached the left edge of the screen
      DODGER.style.left = `${leftDodgerEdge - 4}px`; // if the leftmost edge of the DODGER hasn't reached the left side, it moves
    }                                                // the DODGER 4 pixels to the left
  }

  window.requestAnimationFrame(left) // this is the animation moving the DODGER to the left -- it calls the left function
}
...
function moveDodgerRight() {
  ...
  function right() {
    ...
    if (leftDodgerEdge < (GAME_WIDTH - 40)) { // this is checking to see if the leftmost edge of the DODGER has reached 40 pixels from the right edge of the screen
      DODGER.style.left = `${leftDodgerEdge + 4}px`; // if the leftmost edge of the DODGER hasn't reached 40 pixels from the right side, it moves
    }                                                // the DODGER 4 pixels to the right
  }
  window.requestAnimationFrame(right); // this is the animation moving the DODGER to the right -- it calls the right function
}
